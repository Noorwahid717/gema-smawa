  ...
  const fetchAssignment = useCallback(async () => {
    try {
      ...
      if (submissionResponse.ok) {
        ...
        const userSubmission = submissionData.data

        if (userSubmission) {
          setSubmission(userSubmission)
          setHtml(userSubmission.html || currentAssignment.starterHtml || '')
          setCss(userSubmission.css || currentAssignment.starterCss || '')
          setJs(userSubmission.js || currentAssignment.starterJs || '')
        } else {
          // Initialize with starter code
          setHtml(currentAssignment.starterHtml || '')
          setCss(currentAssignment.starterCss || '')
          setJs(currentAssignment.starterJs || '')
        }
      }
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className={`px-2 py-1 text-xs rounded-full ${
                          assignment.difficulty === 'BEGINNER' ? 'bg-blue-100 text-blue-800' :
                          assignment.difficulty === 'INTERMEDIATE' ? 'bg-orange-100 text-orange-800' :
            ...
            <div className="p-4">
              {activeTab === 'html' && (
                <textarea
                  ...
                  className="w-full h-96 font-mono text-sm border-0 focus:ring-0 resize-none"
