...
export async function POST(request: Request) {
  ...
  try {
    const task = await prisma.codingLabTask.upsert({
      where: {
        title_classLevel: {
          title,
          classLevel
    ...
    console.error('Failed to upsert coding lab task', error)
    return NextResponse.json({ error: 'Failed to save coding lab task' }, { status: 500 })
  }
}